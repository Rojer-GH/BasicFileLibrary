 ...
 * ASCII encoding (total of 127 characters). You can write/read: Integers,
 * Booleans, Floats, Doubles and normal Strings. You can change the encoding of
 * every file to prevent "normal" people from reading your files.
 * 
 * @author Rojer
 * @version 7.04.2019
 */
...
public class EditableFile {
	 ...
	 * List of types
	 */
	protected static final String TYPE_INT = "INT", TYPE_CHAR = "CHAR", TYPE_BOOLEAN = "BOOLEAN",
			TYPE_FLOAT_DOUBLE = "FLOAT/DOUBLE", TYPE_STRING = "STRING";
	...
	public EditableFile(String path, String fileName, int offsetEncoding) {
		this(path + "/" + fileName, offsetEncoding);
	}
	 ...
	 * @param path     the path to the file
	 * @param fileName the name of the file (with extension!)
	 * @param encoding the custom list of encoded characters (only the characters
	 *                 that should be encoded differently)
	 */
	public EditableFile(String path, String fileName, HashMap<Character, Character> encoding) {
		this(path + "/" + fileName, encoding);
	}
	...
	public EditableFile(String path, String fileName, HashMap<Character, Character> encoding, int offsetEncoding) {
		this(path + "/" + fileName, encoding, offsetEncoding);
	}
	 ...
	 */
	protected void addEncodingChar(char target, char encoding) {
	 ...
	 * 
	 * @throws DuplicateEncodingException when a minimum of 2 chars have the same
	 *                                    encoding
	 */

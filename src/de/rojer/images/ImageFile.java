...
public class ImageFile {
	...
	public ImageFile(String path, String fileName, int width, int height) {
		try {
			...
			this.image = toBufferedImage(
					ImageIO.read(new File(path + "/" + fileName)).getScaledInstance(newWidth, newHeight, 0));
			this.width = this.image.getWidth();
			this.height = this.image.getHeight();
			this.pixels = this.image.getRGB(0, 0, newWidth, newHeight, null, 0, newWidth);
		} catch (IOException e) {
	...
	public ImageFile(String completePath, int width, int height) {
		try {
			...
			newHeight = (height <= 0) ? image.getHeight() : height;
			this.image = toBufferedImage(
					ImageIO.read(new File(completePath)).getScaledInstance(newWidth, newHeight, 0));
			this.width = this.image.getWidth();
			this.height = this.image.getHeight();
			this.pixels = this.image.getRGB(0, 0, newWidth, newHeight, null, 0, newWidth);
		} catch (IOException e) {
	 ...
	 */
	protected void checkForGoodDimensions(int posX, int posY, int width, int height)
		...
		} else if (height <= 0 || height + posY > this.image.getHeight()) {
			throw new ImpossibleDimensionsException("" + height, "1 - " + (this.image.getHeight() - posY));
		}
	}
	 ...
	 * @throws ImpossibleDimensionsException when the wanted values are not suited
	 */
	protected void checkForGoodScaling(float scaleX, int width, float scaleY, int height)
			throws ImpossibleDimensionsException {
		if (scaleX * width <= 0) {
			throw new ImpossibleDimensionsException("" + scaleX + " (at a width of " + width + " )",
					"Min. " + (float) (1 / (float) width));
		} else if (scaleY * height <= 0) {
			throw new ImpossibleDimensionsException("" + scaleY + " (at a height of " + width + " )",
					"Min. " + (float) (1 / (float) height));
		}
	}
